...
class UploadDocumentUseCase:
    ...
    async def execute(
        self, file_content, filename: str, content_type: str, user_id: str, domain: str
    ) -> DocumentEntity:
...
class ListUserDocumentsUseCase:
    ...
    async def execute(
        self, user_id: str, domain: str = None, skip: int = 0, limit: int = 100
    ) -> List[DocumentEntity]:
        if domain:
            return await self.document_repository.find_by_user_and_domain(
                user_id, domain, skip, limit
            )
