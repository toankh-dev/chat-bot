-- Embedding Index Table (for vector metadata, not raw vectors)
...
COMMENT ON COLUMN "embedding_index"."embedding_type" IS 'Type of embedding (e.g., openai, bedrock, custom)';
COMMENT ON COLUMN "embedding_index"."vector_id" IS 'ID in vector store (ChromaDB/S3)';

ALTER TABLE "embedding_index" ADD FOREIGN KEY ("user_id") REFERENCES "users" ("id") ON DELETE CASCADE;
ALTER TABLE "embedding_index" ADD FOREIGN KEY ("chatbot_id") REFERENCES "chatbots" ("id") ON DELETE SET NULL;
...
COMMENT ON COLUMN "ingestion_jobs"."provider" IS 'slack, gitlab, backlog, etc.';
COMMENT ON COLUMN "ingestion_jobs"."status" IS 'pending, running, completed, failed';
COMMENT ON COLUMN "ingestion_jobs"."source" IS 'Source identifier (e.g., channel, repo)';
COMMENT ON COLUMN "ingestion_jobs"."details" IS 'Job metadata/config';
COMMENT ON COLUMN "ingestion_jobs"."error_message" IS 'Error details if failed';

ALTER TABLE "ingestion_jobs" ADD FOREIGN KEY ("user_id") REFERENCES "users" ("id") ON DELETE SET NULL;
CREATE TABLE "users" (
  "id" INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
